# ============================================================
# MATCH_SYSTEM.PY - RANKED/APOSTADO (OTIMIZAÇÃO EXTREMA)
# ============================================================
# Otimizações:
# - active_lobbies com limite rígido + limpeza automática
# - Sem cópias desnecessárias de dicts grandes
# - Embeds montados só quando preciso
# - Reembolso atômico e correto
# - Vitória 1v1 corrigida
# - Tasks controladas (não vazam)
# - Cache de settings via database
# ============================================================
...
# Limite duro de lobbies em memória (Railway free)
_MAX_LOBBIES = 40
_LOBBY_TTL = 900  # 15 min sem ação = purge


class MatchSystem:
    __slots__ = ("db", "config", "bot", "active_lobbies", "match_types", "_last_purge")
    ...
    async def _announce_match(self, channel_id: str, match_id: str, match_data: dict):
        ...
        is_bet = match_data.get("is_betting")
        cat = "💰 APOSTADO" if is_bet else "🏆 RANKED"
        ...
        if is_bet and match_data.get("bet_amount"):
            embed.add_field(name="💰 Aposta", value=str(match_data["bet_amount"]), inline=True)
        embed.add_field(name="👥 Jogadores", value=f"1/{max_p}", inline=True)
        embed.add_field(
            name="📌 Como entrar",
            value=f"`!join {match_id[:6]}`"
                  + ("" if match_data["match_type"] == "1v1" else " team1|team2"),
            inline=False,
        )
    ...
    def _ticket_embed(self, match_data: dict, lobby: dict, settings: dict) -> discord.Embed:
        ...
        footer = settings.get("customization", {}).get("embed_footer", "Rank System")
...
# ============================================================
# VIEW DO MEDIADOR (vitória corrigida)
# ============================================================

class MediatorView(View):
    def __init__(self, match_id: str, match_type: str, is_betting: bool, bot, match_system: MatchSystem):
    ...
    async def _declare_winner(self, interaction: discord.Interaction, winner: Optional[str]):
        ...
        if self.match_type == "1v1":
            # CORRIGIDO: em 1v1, team1 = players[0], team2 = players[1]
            if len(players) >= 2:
                if winner == "team1":
