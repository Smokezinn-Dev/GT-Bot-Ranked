# ============================================================
# MATCH_SYSTEM.PY - COM DROPDOWN DE MAPAS + PAINEL DE TICKETS
# ============================================================
...
class MapSelectView(View):
    """View com dropdown para selecionar mapa e criar partida"""
    ...
    @discord.ui.select(
        ...
        max_values=1,
        options=[
            discord.SelectOption(label="🏰 Arena", value="Arena", description="Mapa padrão", emoji="🏰"),
            discord.SelectOption(label="🌲 Floresta", value="Floresta", description="Terreno difícil", emoji="🌲"),
            discord.SelectOption(label="🏯 Castelo", value="Castelo", description="Batalha épica", emoji="🏯"),
            discord.SelectOption(label="🏜️ Deserto", value="Deserto", description="Visibilidade reduzida", emoji="🏜️"),
            discord.SelectOption(label="🌋 Vulcão", value="Vulcão", description="Terreno perigoso", emoji="🌋"),
            discord.SelectOption(label="❄️ Tundra", value="Tundra", description="Campo aberto", emoji="❄️"),
            discord.SelectOption(label="🌃 Cidade", value="Cidade", description="Combate urbano", emoji="🌃"),
            discord.SelectOption(label="🏝️ Ilha", value="Ilha", description="Batalha na praia", emoji="🏝️"),
        ]
    )
    async def map_select(self, interaction: discord.Interaction, select: Select):
        self.selected_map = select.values[0]
        
        # Atualiza o embed com o mapa escolhido
        embed = discord.Embed(
        ...
        # Desativa o dropdown e mostra botão de confirmar
...
class TicketPanelView(View):
    ...
    async def open_ticket(self, interaction: discord.Interaction, button: Button):
        ...
        # Abre o dropdown de seleção de mapa
        embed = discord.Embed(
            title="🗺️ Criar Nova Partida",
            description="Selecione o mapa e o tipo de partida abaixo:",
            color=0x00ff00,
            timestamp=datetime.utcnow()
        )
        embed.add_field(
            name="📋 Instruções",
            value=(
                "1. Selecione o mapa no dropdown\n"
                "2. Escolha o tipo de partida\n"
                "3. Defina se é apostado ou ranked\n"
                "4. Clique em confirmar"
            ),
            inline=False
        )
        
        view = TicketCreateView(self.ticket_system, self.guild_id, interaction.user.id)
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)


class TicketCreateView(View):
    """View para criar ticket com seleção de mapa e tipo"""
    def __init__(self, ticket_system, guild_id: str, user_id: int):
        super().__init__(timeout=120)
        self.ticket_system = ticket_system
        self.guild_id = guild_id
        self.user_id = user_id
        self.selected_map = "Arena"
        self.selected_type = "1v1"
    ...
    async def map_select(self, interaction: discord.Interaction, select: Select):
        ...
        embed.set_footer(text="Continue configurando abaixo...")
    ...
    @discord.ui.select(
        ...
        max_values=1,
        options=[
            discord.SelectOption(label="⚔️ 1v1", value="1v1", description="Duelo individual", emoji="⚔️"),
            discord.SelectOption(label="👥 2v2", value="2v2", description="Duplas", emoji="👥"),
            discord.SelectOption(label="👨‍👩‍👦 3v3", value="3v3", description="Times completos", emoji="👨‍👩‍👦"),
        ]
    )
    async def type_select(self, interaction: discord.Interaction, select: Select):
        ...
        embed = discord.Embed(
            ...
            description=f"**Tipo:** {self.selected_type}",
            color=0x00ff00
        )
        embed.set_footer(text="Continue configurando abaixo...")
        await interaction.response.edit_message(embed=embed)
    ...
    @discord.ui.select(
        ...
        max_values=1,
        options=[
            discord.SelectOption(label="🏆 Ranked", value="ranked", description="Partida ranqueada", emoji="🏆"),
            discord.SelectOption(label="💰 Apostado", value="betting", description="Partida com aposta", emoji="💰"),
        ]
    )
    async def mode_select(self, interaction: discord.Interaction, select: Select):
        self.is_betting = select.values[0] == "betting"
        
        if self.is_betting:
            embed = discord.Embed(
                title="💰 Modo Apostado",
                description="Digite o valor da aposta no chat.\nExemplo: `100`",
                color=0xffd700
            )
            await interaction.response.edit_message(embed=embed)
            ...
            def check(m):
                return m.author.id == self.user_id and m.channel == interaction.channel and m.content.isdigit()
            
            try:
                msg = await interaction.followup.wait_for("message", check=check, timeout=30)
                self.bet_amount = int(msg.content)
                await msg.delete()
                
                embed = discord.Embed(
                    title="✅ Configuração Completa!",
                    description=f"**Mapa:** {self.selected_map}\n**Tipo:** {self.selected_type}\n**Modo:** {'💰 Apostado' if self.is_betting else '🏆 Ranked'}\n**Aposta:** {self.bet_amount if self.is_betting else 'N/A'}",
                    color=0x00ff00,
                    timestamp=datetime.utcnow()
                )
                embed.set_footer(text="Clique em '✅ Criar Partida' para finalizar")
                
                # Mostra botão de criar
                create_view = TicketConfirmView(self)
                await interaction.followup.send(embed=embed, view=create_view)
            except TimeoutError:
...
class TicketConfirmView(View):
    """View de confirmação para criar partida via ticket"""
    def __init__(self, parent_view: TicketCreateView):
        super().__init__(timeout=60)
        self.parent = parent_view

    @discord.ui.button(label="✅ Criar Partida", style=discord.ButtonStyle.success, emoji="✅")
    async def create_button(self, interaction: discord.Interaction, button: Button):
        result = await self.parent.ticket_system.match_system.create_lobby(
            guild_id=self.parent.guild_id,
            channel_id=str(interaction.channel.id),
            author_id=str(self.parent.user_id),
            match_type=self.parent.selected_type,
            map_name=self.parent.selected_map,
            is_betting=self.parent.is_betting,
            bet_amount=self.parent.bet_amount,
        )
        ...
        embed = discord.Embed(
            ...
            description=f"**ID:** `{result['match_id'][:6]}`\n**Tipo:** {self.parent.selected_type}\n**Mapa:** {self.parent.selected_map}\n**Modo:** {'💰 Apostado' if self.parent.is_betting else '🏆 Ranked'}\n**Aposta:** {self.parent.bet_amount if self.parent.is_betting else 'N/A'}",
            color=0x00ff00,
            timestamp=datetime.utcnow()
        )
        embed.set_footer(text=f"Use !join {result['match_id'][:6]} para entrar")
...
class MatchSystem:
    __slots__ = ("db", "config", "bot", "active_lobbies", "match_types", "_last_purge", "lobby_messages")
    ...
    def _create_lobby_embed(self, lobby: dict) -> discord.Embed:
        """Cria embed customizável para o lobby"""
    ...
    async def _announce_match(self, channel_id: str, match_id: str, lobby: dict):
        """Envia embed do lobby com botões"""
