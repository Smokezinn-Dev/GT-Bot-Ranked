# ============================================================
# EMBED_BUILDER.PY - PAINEL MODERNO v9.0 (COMPLETO)
# ============================================================
...
def convert_emoji_input(emoji_input: str, guild: discord.Guild) -> Optional[str]:
    """
    Converte entrada de emoji para formato utilizável.
    
    Exemplos:
    - :blockdash: -> <:blockdash:123456789>
    - <:blockdash:123456789> -> <:blockdash:123456789>
    - 🏃 -> 🏃
    - blockdash -> <:blockdash:123456789> (se existir)
    """
    if not emoji_input or not guild:
        return "🗺️"
    
    emoji_input = emoji_input.strip()
    
    # Já é um emoji customizado
    if is_custom_emoji(emoji_input):
        return emoji_input
    
    # É um código :nome:
    if is_emoji_code(emoji_input):
        emoji_name = emoji_input[1:-1]  # Remove os :
        for emoji in guild.emojis:
            if emoji.name.lower() == emoji_name.lower():
                return str(emoji)
        # Tenta com underline removido
        emoji_name_clean = emoji_name.replace('_', '')
        for emoji in guild.emojis:
            if emoji.name.lower().replace('_', '') == emoji_name_clean:
                return str(emoji)
        return None
    
    # É um nome simples (ex: "blockdash")
    if is_simple_name(emoji_input):
        for emoji in guild.emojis:
            if emoji.name.lower() == emoji_input.lower():
                return str(emoji)
        # Tenta com underline
        for emoji in guild.emojis:
            if emoji.name.lower().replace('_', '') == emoji_input.lower().replace('_', ''):
                return str(emoji)
        return None
    
    # É um emoji Unicode ou texto
    return emoji_input
...
def get_emoji_display(emoji: str) -> str:
    ...
    if is_custom_emoji(emoji):
        # Extrai o nome do emoji para exibição
...
# ============================================================
# MODAL - MAPA COM IMAGEM (COM SUPORTE A EMOJIS)
# ============================================================
...
class MapWithImageModal(Modal):
    ...
    def __init__(self, panel_view, option_index: Optional[int] = None, 
        ...
        self.emoji_input = TextInput(
            label="🎨 Emoji ou Código do Mapa",
            placeholder="Use :nome_do_emoji: ou cole o emoji (ex: :blockdash:)",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 🔥 CONVERTE O EMOJI
        emoji = convert_emoji_input(emoji_input, interaction.guild)
        
        if emoji is None:
            # Lista emojis disponíveis para ajudar
            emojis_list = [f":{e.name}:" for e in interaction.guild.emojis[:10]]
            emojis_text = ", ".join(emojis_list) if emojis_list else "Nenhum emoji customizado"
            
            return await interaction.response.send_message(
                f"❌ Emoji `{emoji_input}` não encontrado!\n\n"
                "**📌 Como criar um emoji customizado:**\n"
                "1. **Configurações do Servidor > Emojis**\n"
                "2. **Carregar Emoji**\n"
                "3. Envie a imagem do mapa\n"
                "4. Dê um nome (ex: `blockdash`)\n"
                "5. Use `:blockdash:` no campo!\n\n"
                f"**📋 Emojis disponíveis:** {emojis_text}\n\n"
                "**💡 Ou use um emoji Unicode normal:** 🏃 ⭐ 🔥",
                ephemeral=True
            )
...
class MapActionModal(Modal):
    ...
    def __init__(self, parent_view, option_index: int, 
        ...
        self.emoji_input = TextInput(
            label="🎨 Emoji ou Código",
            default=current_emoji or "🗺️",
            placeholder="Use :nome_do_emoji: ou cole o emoji",
            style=discord.TextStyle.short,
            max_length=100,
            required=False
        )
...
class QueuePublishView(View):
            ...
            if isinstance(child, discord.ui.Select):
                ...
                if self.options:
                    for opt in self.options[:25]:
                        ...
                        # 🔥 O EMOJI JÁ ESTÁ NO FORMATO CORRETO
    ...
    def build_publish_embed(self) -> discord.Embed:
        ...
        if self.options and self.match_system:
            ...
            for opt in self.options[:25]:
                ...
                # Mostra o emoji (customizado ou Unicode)
    ...
    async def select_map(self, interaction: discord.Interaction, select: Select):
        ...
        # 🔥 ATUALIZA O EMBED COM O MAPA SELECIONADO
...
class OptionsPanelView(View):
    ...
    async def select_option(self, interaction: discord.Interaction, select: Select):
        ...
        # 🔥 MOSTRA A IMAGEM DO MAPA QUANDO SELECIONADO
        embed = self.parent_view.build_options_embed()
        if option.get("image"):
            embed.set_image(url=option["image"])
            embed.add_field(
                name="🖼️ Visualizar",
                value=f"Imagem do mapa **{option.get('name')}**",
                inline=False
            )
    ...
    def build_options_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "• Digite `:nome_do_emoji:` (ex: `:blockdash:`)\n"
                "• Ou cole o emoji completo `<:blockdash:123456789>`\n"
                "• Ou use emojis Unicode: 🏃 ⭐ 🔥\n\n"
        ...
        if self.options:
            ...
            for i, opt in enumerate(self.options, 1):
                ...
                # Mostra o emoji (pode ser customizado)
...
def setup_embed_builder(bot):
    ...
    async def painel_cmd(ctx):
        ...
        embed.add_field(
            name="📌 Como adicionar imagens e emojis",
            value=(
                "**1️⃣ URL Direta:**\n"
                ...
                "• Cole no campo URL\n\n"
                "**3️⃣ Emojis Customizados:**\n"
                "• Crie um emoji no servidor (Config > Emojis)\n"
                "• Use `:nome_do_emoji:` no campo emoji\n"
                "• Exemplo: `:blockdash:` ou `:mapa_arena:`\n\n"
                "✅ **URLs que funcionam:** Imgur, Discord, Pinterest\n"
                "❌ **Não funcionam:** Instagram, Facebook, TikTok"
            ),
            inline=False
