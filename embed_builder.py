# ============================================================
# EMBED_BUILDER.PY - PAINEL MODERNO v5.1
# ============================================================
...
# ============================================================
# VALIDADOR INTELIGENTE DE URL DE IMAGEM
# ============================================================

# Padrões de URL de imagem (suporta praticamente qualquer serviço)
IMAGE_URL_PATTERNS = [
    # Extensões comuns
    r'\.(png|jpg|jpeg|gif|webp|bmp|svg|ico|tiff|tif)(\?.*)?$',
    r'\.(png|jpg|jpeg|gif|webp|bmp|svg|ico|tiff|tif)/',
...
def is_valid_image_url_quick(url: str) -> bool:
    """Validação rápida de URL de imagem (sem fazer request)"""
...
async def verify_image_url_async(url: str, timeout: int = 5) -> bool:
    """
    Verifica se uma URL realmente retorna uma imagem fazendo uma requisição HEAD.
    Mais lento mas mais preciso.
    """
    if not url:
        return False
    
    # Primeiro faz validação rápida
    if not is_valid_image_url_quick(url):
        return False
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout_obj) as session:
            async with session.head(url, allow_redirects=True) as response:
                ...
                return content_type.startswith(('image/', 'image')) or 'image' in content_type
    except Exception:
        # Se falhar, aceita a URL baseado na validação rápida
        return is_valid_image_url_quick(url)


def is_valid_image_url(url: str, fast: bool = True) -> bool:
    """
    Verifica se é uma URL de imagem válida.
    - fast=True: usa apenas validação por padrões (rápido)
    - fast=False: faz requisição HEAD (mais preciso, mais lento)
    """
    if fast:
        return is_valid_image_url_quick(url)
    else:
        # Versão síncrona para uso em contexto não assíncrono
        try:
            import requests
            ...
            if response.status_code != 200:
                return False
            content_type = response.headers.get('Content-Type', '')
            return content_type.startswith(('image/', 'image'))
        except Exception:
            return is_valid_image_url_quick(url)


def extract_image_info(url: str) -> dict:
    """Extrai informações de uma URL de imagem"""
    is_valid = is_valid_image_url_quick(url)
    return {
        "url": url,
        "is_valid": is_valid,
        "service": _detect_service(url),
        "filename": url.split('/')[-1].split('?')[0] if url else None,
    }


def _detect_service(url: str) -> str:
    """Detecta o serviço de hospedagem da imagem"""
    url_lower = url.lower()
    services = {
...
class MapWithImageModal(Modal):
    ...
    def __init__(self, panel_view, option_index: Optional[int] = None, 
        ...
        self.image_input = TextInput(
            label="🖼️ URL da Imagem (opcional - qualquer serviço!)",
            placeholder="Ex: https://pinimg.com/... ou https://i.imgur.com/...",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Validação de imagem melhorada
        if image:
            is_valid = is_valid_image_url_quick(image)
            if not is_valid:
                # Tenta validar com requisição (mais lento mas mais preciso)
                is_valid = await verify_image_url_async(image)
            
            if not is_valid:
                return await interaction.response.send_message(
                    "❌ URL de imagem inválida!\n\n"
                    "**Formatos suportados:** PNG, JPG, GIF, WEBP, SVG, BMP, ICO, TIFF\n"
                    "**Serviços suportados:** Pinterest, Imgur, Discord CDN, Gyazo, ImgBB, "
                    "PostImage, GitHub, Reddit, Twitter, Facebook, Instagram, Google Drive, "
                    "Dropbox, Steam, Cloudflare, AWS S3, CloudFront, Giphy, Tenor e qualquer CDN.",
                    ephemeral=True
                )
        ...
        if image:
            ...
            option_data["service"] = _detect_service(image)
...
class ColorPickerView(View):
    ...
    def _add_color_buttons(self):
        """Adiciona botões de cores em linhas de 4"""
...
# ============================================================
# VIEW PRINCIPAL - EMBED BUILDER MODERNO
# ============================================================
...
class EmbedBuilderView(View):
    ...
    # ============================================================
    # BOTÕES - MENU PRINCIPAL (ORGANIZADOS EM LINHAS)
    # ============================================================
    ...
    # ============================================================
    # MÉTODOS DO EMBED - VERSÃO FINAL (BONITA)
    # ============================================================
    ...
    def build_final_embed(self) -> discord.Embed:
        ...
        # Banner de informações (sem campos desnecessários)
        ...
        # Lista de mapas com imagens (se tiver)
        if self.options:
            map_lines = []
    ...
    def build_options_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "📌 **Suporte a qualquer serviço:** Pinterest, Imgur, Discord CDN, "
                "Gyazo, ImgBB, PostImage, GitHub, Reddit, Twitter, Facebook, "
                "Instagram, Google Drive, Dropbox, Steam, Cloudflare, AWS S3, "
                "CloudFront, Giphy, Tenor e qualquer CDN."
            ),
            color=color,
            timestamp=datetime.utcnow()
        )
...
# ============================================================
# VIEW - PUBLISH (COM IMAGENS NOS MAPAS)
# ============================================================

class QueuePublishView(View):
    """View da mensagem publicada com suporte a imagens nos mapas"""
    ...
    def _get_map_display(self, opt: dict) -> str:
        """Retorna a representação visual do mapa"""
        name = opt.get("name", "Mapa")
        emoji = opt.get("emoji", "🗺️")
        image = opt.get("image")
        
        if image:
            return f"{emoji} **{name}** 🖼️"
        return f"{emoji} `{name}`"
    ...
    def build_publish_embed(self) -> discord.Embed:
        ...
        # Mapas disponíveis com status da fila
...
class OptionsPanelView(View):
    ...
    async def add_stumble_maps(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            # Mapas Stumble com imagens (usando URLs de exemplo do Imgur)
            STUMBLE_MAPS = [
...
class MapActionModal(Modal):
    def __init__(self, parent_view, option_index: int, 
        ...
        self.image_input = TextInput(
            label="🖼️ URL da Imagem (deixe vazio para remover)",
...
class ThumbnailModal(Modal):
    def __init__(self, builder, current_url: str = ""):
        ...
        self.url_input = TextInput(
            ...
            placeholder="https://pinimg.com/... ou https://i.imgur.com/...",
            default=current_url,
            style=discord.TextStyle.short,
            max_length=500,
            required=False
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if url:
            is_valid = is_valid_image_url_quick(url)
            if not is_valid:
                is_valid = await verify_image_url_async(url)
...
def setup_embed_builder(bot):
    ...
    async def painel_cmd(ctx):
        ...
        # Adiciona um banner visual
        embed.add_field(
            name="✨ Dica",
            value="Mapas com **imagem** (URL) aparecem com destaque no painel!\nSuporta **Pinterest, Imgur, Discord CDN, Gyazo** e muitos outros!",
            inline=False
        )
