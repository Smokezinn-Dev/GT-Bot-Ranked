# ============================================================
# MAIN.PY - GT BOT RANKED (OTIMIZAÇÃO EXTREMA + SEGURO)
# ============================================================
...
from admin_commands import AdminCommands, clear_permission_cache
        ...
        while not self.is_closed() and not self._shutdown:
            ...
            try:
                ...
                if self.match_system:
                    self.match_system._purge_stale()
                    self.match_system._purge_stale_queues()
                gc.collect()
            except Exception as e:
...
async def memory_cmd(ctx):
    ...
    embed.add_field(
        name="📋 Filas",
        value=f"**Combinações ativas:** {len(bot.match_system.queues)}\n**Jogadores na fila:** {len(bot.match_system.queued_players)}",
        inline=True
    )
    await ctx.send(embed=embed)
...
async def help_cmd(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            f"`{PREFIX}setmediator @cargo` — Define (substitui) o mediador\n"
            f"`{PREFIX}addmediator / removemediator @cargo` — Ajusta mediadores\n"
            f"`{PREFIX}rewards 1v1 top3 @cargo 500` — Premiação de ranking\n"
            f"`{PREFIX}distributerewards 1v1` — Distribui a premiação\n"
    ...
    embed.add_field(
        ...
        value=(
            f"`{PREFIX}painel` — Painel completo com botões e selects\n"
            f"`{PREFIX}embed` — Construtor de embed (admin)"
        ),
        inline=False
    )
