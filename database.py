...
# ============================================================
# CACHE LRU COM MONITORAMENTO
# ============================================================

class _LRU:
    __slots__ = ("maxsize", "ttl", "_data", "_hits", "_misses")
...
class _ConnectionPoolMonitor:
    """Fecha a conexão com o Mongo quando ela fica realmente ociosa
    (sem nenhuma query por muito tempo), liberando as threads/handles
    internas do driver. Reconecta sozinho na próxima chamada."""
    __slots__ = ("_last_check", "_check_interval", "_idle_timeout")
    ...
    def check_and_cleanup(self):
        """Só fecha o client se ninguém usou o banco por _idle_timeout segundos"""
        now = time.monotonic()
...
def get_mongo_client():
    global _client, _last_db_activity
    _last_db_activity = time.monotonic()
...
def cleanup_idle_connection():
    """Chamado periodicamente pelo optimizer. Fecha o client do Mongo
    só se ele estiver ocioso há muito tempo (ver _ConnectionPoolMonitor)."""
...
# Registrar para fechar na saída
