...
class _ConnectionPoolMonitor:
    """Monitora e gerencia o pool de conexões"""
    __slots__ = ("_last_check", "_interval", "_max_connections")
    
    def __init__(self, interval: int = 60, max_connections: int = 3):
    ...
    def check_and_cleanup(self):
        """Verifica e limpa conexões ociosas"""
        now = time.monotonic()
    ...
    with _connection_lock:
        ...
        for attempt in range(3):
            try:
                ...
                # Remove event listeners para economia de memória
                if hasattr(_client, 'event_listeners'):
                    _client.event_listeners = []
                return _client
